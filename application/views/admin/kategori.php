
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		<div class="row">
			<ol class="breadcrumb">
				<li><a href="#">
					<em class="fa fa-home"></em>
				</a></li>
				<li class="active">Kategori</li>
			</ol>
		</div><!--/.row-->

		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">Kategori</h1>
			</div>
		</div><!--/.row-->

		<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default ">
					<?php if(!empty($this->session->flashdata('info'))): ?>
						<div class="panel-heading text-center" style="color:blue; border:1px solid blue;">
							<?= $this->session->flashdata('info') ?>
						</div>
					<?php endif ?>
					<div class="panel-heading">
						Kategori
						<span class="pull-right clickable panel-toggle panel-button-tab-left"><em class="fa fa-toggle-up"></em></span>
					</div>
					<div class="panel-body timeline-container">
						<table class="table">
							<tr>
								<th>No</th>
								<th>Nama</th>
								<th>Action</th>
							</tr>
							<?php $i = 1 ?>
							<?php foreach ($kategoris as $kategori): ?>
							<tr>
								<td><?= $i ?></td>
								<td><?= $kategori->nama_kategori ?></td>
								<td>
									<a href="<?= base_url('admin-category/'.$kategori->id_kategori) ?>" class="btn btn-primary pull-left" style="color:white;">Detail</a>
									<a href="<?= base_url('admin-category/edit/'.$kategori->id_kategori) ?>" class="btn btn-warning pull-left" style="color:white;">Edit</a>
									<?= form_open('admin-category/delete', ['class' => 'pull-left']) ?>
									<?= form_hidden('id_kategori', $kategori->id_kategori) ?>
									<?= form_submit(null, 'Hapus', ['class' => 'btn btn-danger']) ?>
									<?= form_close() ?>
								</td>
							</tr>
							<?php $i++ ?>
						<?php endforeach ?>
						</table>
					</div>
				</div>

				<div class="panel panel-default">
					<div class="panel-heading">
						Tambah Kategori
						<span class="pull-right clickable panel-toggle panel-button-tab-left"><em class="fa fa-toggle-up"></em></span></div>
					<div class="panel-body">
						<?= form_open('admin-category/post', ['class' => 'form-horizontal']) ?>

						<?= validation_errors() ?>

			      <?php if(!empty($this->session->flashdata('msg'))): ?>
			        <p class="text-center" style="color:blue; border:1px solid blue; padding:7px"><?= $this->session->flashdata('msg') ?></p>
			      <?php endif ?>

							<fieldset>
								<!-- Name input-->
								<div class="form-group" style="margin-top:10px">
									<label class="col-md-3 control-label" for="name">Nama Kategori</label>
									<div class="col-md-9">
										<?= form_input('nama_kategori', $input->nama_kategori, ['class' => 'form-control', 'placeholder' => 'Nama Kategori']) ?>
									</div>
								</div>

								<!-- Desc body -->
								<div class="form-group">
									<label class="col-md-3 control-label" for="message">Deskripsi Kategori</label>
									<div class="col-md-9">
										<?= form_textarea('deskripsi_kategori', $input->deskripsi_kategori, ['class' => 'form-control', 'placeholder' => 'Deskripsi Kategori']) ?>
									</div>
								</div>

								<!-- Form actions -->
								<div class="form-group">
									<div class="col-md-12 widget-right">
											<?= form_submit(null, 'Tambah', ['class' => 'btn btn-default btn-md pull-right']) ?>
									</div>
								</div>
							</fieldset>
						<?= form_close() ?>
					</div>
				</div>
			</div><!--/.col-->
		</div><!--/.row-->
	</div>	<!--/.main-->


<script src="js/jquery-1.11.1.min.js"></script>
	<script src="js/bootstrap.min.js"></script>
	<script src="js/chart.min.js"></script>
	<script src="js/chart-data.js"></script>
	<script src="js/easypiechart.js"></script>
	<script src="js/easypiechart-data.js"></script>
	<script src="js/bootstrap-datepicker.js"></script>
	<script src="js/custom.js"></script>

</body>
</html>
